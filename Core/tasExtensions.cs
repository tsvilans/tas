...
{
    /// <summary>
    /// tasTools extension methods for existing Rhino types
    /// </summary>
    public static class ExtensionMethods
    {
        ...
        public static double DistanceToSq(this Point3d a, Point3d b)
        {
        ...
        public static List<Polyline> PlaneSlice(this Polyline poly, Plane p, bool clip = false, bool close = false)
        {
            ...
            for (int i = 0; i < XSeg.Count; ++i)
            {
                ...
                index_point = Util.PlaneLineIntersection(new Line(poly[XSeg[i].Item1], poly[XSeg[i].Item2]), p);
                pts.Add(index_point);
        ...
        /// <summary>
        /// Triangulate mesh in-place. Code ripped + adapted from http://james-ramsden.com/triangulate-a-quad-mesh-in-c/
        /// </summary>
        /// <param name="M">Mesh to triangulate.</param>
        public static void Triangulate(this Mesh M)
        {
            M.Faces.ConvertQuadsToTriangles(); // duh...
            return;
            /*
            //Mesh m = M.Duplicate() as Mesh;
            ...
            for (int i = 0; i < facecount; ++i)
            {
                ...
                if (mf.IsQuad)
                {
                    ...
                    if (dist1 > dist2)
                    {
                        M.Faces.AddFace(mf.A, mf.B, mf.D);
                        M.Faces.AddFace(mf.B, mf.C, mf.D);
                    }
                    else
                    {
                        M.Faces.AddFace(mf.A, mf.B, mf.C);
                        M.Faces.AddFace(mf.A, mf.C, mf.D);
                    }
                }
            }

            var newfaces = new List<MeshFace>();
            foreach (var mf in M.Faces)
            {
        ...
        /// <summary>
        /// Triangulate and return new mesh.
        /// </summary>
        /// <param name="M">Mesh to triangulate.</param>
        /// <returns></returns>
        public static Mesh Triangulated(this Mesh M)
        {
        ...
        public static PointCloud PointCloud(this Mesh m, int N)
        {
            ...
            for (int i = 0; i < M.Faces.Count; ++i)
            {
                ...
                double area = Util.TriangleArea(A, B, C);
                ...
                for (int j = 0; j < npf; ++j)
                {
                    Points.Add(Util.PointOnTriangle(A, B, C, rnd.NextDouble(), rnd.NextDouble()));
                    Normals.Add(Normal);
                }
            }
        ...
        public static Curve Relax(this Curve c, double factor, bool make_single_curved = false, Plane? projection_plane = null, bool rebuild = false)
        {
            ...
            for (int i = 0; i < CtrlPts.Count; ++i)
            {
                ...
                if (make_single_curved)
                    pt = Util.ProjectToPlane(pt, plane);
        ...
        #region CarveSharp EXTENSION METHODS
